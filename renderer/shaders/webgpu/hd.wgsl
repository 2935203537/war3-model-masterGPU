...
@vertex fn vs(
    ...
    var w: vec4f = in.boneWeight;
    let ws: f32 = w.x + w.y + w.z + w.w;
    if (ws > 0.0) {
        w = w / ws;
    }
    var sum: mat4x4f = mat4x4f();

    sum += vsUniforms.nodesMatrices[in.skin[0]] * w[0];
    sum += vsUniforms.nodesMatrices[in.skin[1]] * w[1];
    sum += vsUniforms.nodesMatrices[in.skin[2]] * w[2];
    sum += vsUniforms.nodesMatrices[in.skin[3]] * w[3];
