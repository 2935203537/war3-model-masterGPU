...
class TOutputStream {
    constructor(cbOutLength) {
        this.cbOutLength = cbOutLength;
        this.pbOutBuffer = [];
        this.bitBuffer = 0;
        this.bitCount = 0;
        this.pos = 0;
...
;
var TInsertPoint;
(function (TInsertPoint) {
    TInsertPoint[TInsertPoint["insertAfter"] = 1] = "insertAfter";
    TInsertPoint[TInsertPoint["insertBefore"] = 2] = "insertBefore";
})(TInsertPoint || (TInsertPoint = {}));
;
...
class THuffmanTree {
    ...
    constructor() {
        this.itemsByByte = new Array(0x102);
        this.quickLinks = new Array(LINK_ITEM_COUNT).fill(0).map(() => ({
            validValue: 0,
            validBits: 0,
            pItem: null,
            decompressedValue: 0,
    ...
    findHigherOrEqualItem(pItem, weight) {
    ...
    createNewItem(decompressedValue, weight, insertPoint) {
    ...
    fixupItemPosByWeight(pNewItem, maxWeight) {
    ...
    buildTree(compressionType) {
        ...
        let pChildHi;
        let weightTable;
        let maxWeight; // [ESP+10] - The greatest character found in table
    ...
    incWeightsAndRebalance(pItem) {
        let pHigherItem; // A previous item with greater or equal weight
        let pChildHi; // The higher-weight child
        let pChildLo; // The lower-weight child
        let pParent;
    ...
    compress(os, pvInBuffer, compressionType) {
        let pos = 0;
...
function decodeHuffman(bytes) {
    const tree = new THuffmanTree();
    return tree.decompress(new TInputStream(bytes.buffer));
}
