...
// NOTE: ModelRenderer uses multisample count = 4 for the main render pass.
// All render pipelines used within that pass must match the same sample count,
// otherwise WebGPU will reject SetPipeline() with an attachment-state mismatch.
    ...
    public initGPUDevice (device: GPUDevice): void {
        ...
        const createPipeline = (name: string, blend: GPUBlendState, depth: GPUDepthStencilState) => {
            return device.createRenderPipeline({
                ...
                depthStencil: depth,
                multisample: {
                    count: WEBGPU_MULTISAMPLE_COUNT
                }
            });
        };
