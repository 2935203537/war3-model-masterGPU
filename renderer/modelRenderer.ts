...
export class ModelRenderer {
    ...
    /** Enable/disable RibbonEmitter updates+rendering (useful for thumbnail grids). */
    public setRibbonsEnabled(enabled: boolean): void {
        this.ribbonsEnabled = enabled;
    }

    /** Convenience setter for multiple effect flags at once. */
    public setEffectsEnabled(opts: { particles?: boolean; ribbons?: boolean }): void {
        if (typeof opts.particles === 'boolean') this.particlesEnabled = opts.particles;
        if (typeof opts.ribbons === 'boolean') this.ribbonsEnabled = opts.ribbons;
    }
            ...
            for (let orderIndex = 0; orderIndex < geosetOrder.length; ++orderIndex) {
                ...
                if (this.isHD) {
                    ...
                    for (let j = 0; j < material.Layers.length; ++j) {
                        ...
                        const geosetColor = vec3.create();
                        this.findColor(i, geosetColor);
                        ...
                        FSUniformsViews.tVertexAnim.set(tVetexAnim.slice(6, 9), 8);
                        FSUniformsViews.wireframe.set([wireframe ? 1 : 0]);
                        FSUniformsViews.geosetColor.set(geosetColor);
                        FSUniformsViews.geosetAlpha.set([this.rendererData.geosetAlpha[i]]);
                        this.device.queue.writeBuffer(gpuFSUniformsBuffer, 0, FSUniformsValues);
    ...
    private findColor (geosetId: number, out: vec3): vec3 {
        const geosetAnim = this.rendererData.geosetAnims[geosetId];

        if (!geosetAnim || geosetAnim.Color === undefined) {
            return vec3.set(out, 1, 1, 1);
        }

        if (geosetAnim.Color instanceof Float32Array) {
            return vec3.set(out, geosetAnim.Color[0], geosetAnim.Color[1], geosetAnim.Color[2]);
        }

        const interpRes = this.interp.vec3(out, geosetAnim.Color);
        if (interpRes === null) {
            return vec3.set(out, 1, 1, 1);
        }
        return vec3.set(out, interpRes[0], interpRes[1], interpRes[2]);
    }
