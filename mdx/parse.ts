...
function parseRibbonEmitters (model: Model, state: State, size: number): void {
    ...
    while (state.pos < startPos + size) {
        ...
        while (state.pos < emitterStart + emitterSize) {
            ...
            } else if (keyword === 'KRCO') {
                // KRCO - Color animation (sometimes present in custom models)
                emitter.Color = state.animVector(AnimVectorType.FLOAT3);
            } else {
                console.warn('Unknown ribbon emitter chunk:', keyword, '- skipping');
                // Skip unknown chunk instead of throwing error
                state.index += size - 4;
            }
        }
