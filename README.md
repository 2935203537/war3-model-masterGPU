...
## Desktop (Electron + WebGPU) 批量模型显示

这个仓库新增了一个桌面端示例：`desktop/`，用 **Electron + WebGPU** 做批量模型预览（网格缩略图），并支持：
- 选择一个文件夹后，递归扫描 `.mdx/.mdl` 以及常见贴图（`.blp/.dds/.tga/.png/.jpg/.webp`）
- 网格批量显示（默认一次显示前 50 个，可调）
- 搜索过滤、Tile 尺寸调整
- 动画/旋转开关（缩略图渲染默认节流到 ~15FPS）
- 粒子/飘带开关（用于提升批量视图性能）
