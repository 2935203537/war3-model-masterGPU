...
function unsignedRightShift(bitBuffer, bits) {
    if (bits >= 32 || bits < -32) {
        const m = (bits / 32);
        bits = bits - (m * 32);
    }
...
function decompressADPCM(data, channelCount, size) {
