...
@fragment fn fs(
    ...
    if (fsUniforms.replaceableType == 0) {
        ...
        color = vec4f(fsUniforms.replaceableColor, 1.0) * alpha;
    }

    // Apply geosetColor and geosetAlpha
    color = vec4f(color.rgb * fsUniforms.geosetColor, color.a * fsUniforms.geosetAlpha);
